...
**How to apply:** The widget parses the same-origin `/faq?faq-source=1` page with `DOMParser`, caches entries in `localStorage` for one hour, and falls back to the bundled FAQ data on failure. FAQ Accordion sections and answers must use `forceMount` so hidden content remains available in the source DOM. Keep the Telegram fallback link intact.