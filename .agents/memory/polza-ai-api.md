...
Для операций смены фона Polza.ai фактически ограничивает модели Grok Imagine и GPT Image 1.5 форматами `1:1`, `2:3` и `3:2`, даже если общий список `/media` допускает больше значений. Сервер должен нормализовать остальные пропорции по ориентации до ближайшего из этих трёх форматов.

**Why:** запросы этих моделей с исходным `9:16` возвращали `400 BAD_REQUEST` и блокировали смену фона.

**How to apply:** Nano Banana получает исходный поддерживаемый формат, а Grok/GPT — `1:1`, `2:3` или `3:2`; исходное соотношение можно сохранять только для логов и UI.
